...
%SQLCMD% -S %SERVER% %AUTH% -C -d master -i "%SCRIPTS_DIR%P10R01_WS_01_CreateDatabase_AEHM.sql" -b
...
%SQLCMD% -S %SERVER% %AUTH% -C -d %DB% -i "%SCRIPTS_DIR%P10R01_WS_02_CreateTables_AEHM.sql" -b
if errorlevel 1 goto error
...
%SQLCMD% -S %SERVER% %AUTH% -C -d %DB% -i "%SCRIPTS_DIR%P10R01_WS_03_CargaInicial_AEHM.sql" -b
if errorlevel 1 goto error
