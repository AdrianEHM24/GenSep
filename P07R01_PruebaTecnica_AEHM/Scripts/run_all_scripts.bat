...
%SQLCMD% -S %SERVER% %AUTH% -C -d master -i "%SCRIPTS_DIR%P07R01_PruebaTecnica_01_CreateDatabase_AEHM.sql" -b
...
%SQLCMD% -S %SERVER% %AUTH% -C -d %DB% -i "%SCRIPTS_DIR%P07R01_PruebaTecnica_02_CreateTables_AEHM.sql" -b
if errorlevel 1 goto error
...
%SQLCMD% -S %SERVER% %AUTH% -C -d %DB% -i "%SCRIPTS_DIR%P07R01_PruebaTecnica_03_CargaInicial_AEHM.sql" -b
if errorlevel 1 goto error

echo.
echo ============================
echo Ejecutando Scripts 4: SPs
echo ============================

%SQLCMD% -S %SERVER% %AUTH% -C -d %DB% -i "%SCRIPTS_DIR%P07R01_PruebaTecnica_04_SPs_AEHM.sql" -b
if errorlevel 1 goto error
