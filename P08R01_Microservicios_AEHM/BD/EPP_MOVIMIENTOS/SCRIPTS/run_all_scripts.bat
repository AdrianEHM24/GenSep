...
%SQLCMD% -S %SERVER% %AUTH% -C -d master -i "%SCRIPTS_DIR%P08R01_Movimientos_01_CreateDatabase_AEHM.sql" -b
if errorlevel 1 goto error

rem Cambia por el nombre de la bd que crea tu script
set "DB=EPP_Movimientos"
...
%SQLCMD% -S %SERVER% %AUTH% -C -d %DB% -i "%SCRIPTS_DIR%P08R01_Movimientos_02_CreateTables_AEHM.sql" -b
if errorlevel 1 goto error
...
%SQLCMD% -S %SERVER% %AUTH% -C -d %DB% -i "%SCRIPTS_DIR%P08R01_Movimientos_03_CargaInicial_AEHM.sql" -b
if errorlevel 1 goto error
