...
%SQLCMD% -S %SERVER% %AUTH% -C -d master -i "%SCRIPTS_DIR%P16R01_WCF_01_CreateDatabase_AEHM.sql" -b
...
%SQLCMD% -S %SERVER% %AUTH% -C -d %DB% -i "%SCRIPTS_DIR%P16R01_WCF_02_CreateTables_AEHM.sql" -b
if errorlevel 1 goto error
...
%SQLCMD% -S %SERVER% %AUTH% -C -d %DB% -i "%SCRIPTS_DIR%P16R01_WCF_03_CargaInicial_AEHM.sql" -b
if errorlevel 1 goto error
