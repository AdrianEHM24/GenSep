...
%SQLCMD% -S %SERVER% %AUTH% -C -d %DB% -i "%SCRIPTS_DIR%P16R01_WCF_A_LimpiezaData_AEHM.sql" -b
...
%SQLCMD% -S %SERVER% %AUTH% -C -d %DB% -i "%SCRIPTS_DIR%P16R01_WCF_B_DropTables_AEHM.sql" -b
if errorlevel 1 goto error
...
%SQLCMD% -S %SERVER% %AUTH% -C -d master -i "%SCRIPTS_DIR%P16R01_WCF_C_DropDatabase_AEHM.sql" -b
if errorlevel 1 goto error
